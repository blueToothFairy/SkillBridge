...
async function seedTestProject() {
    ...
    const studentIds = usersToDelete.map(u => u.studentProfile?.id).filter(Boolean);
    const smeIds = usersToDelete.map(u => u.smeProfile?.id).filter(Boolean);
    ...
    const studentPasswordHash = await bcryptjs_1.default.hash('Password123!', 10);
    ...
    const smePasswordHash = await bcryptjs_1.default.hash('Password123!', 10);
